...
def extract_last_layer(tar_path, layer_path, diffid_path):
  ...
  try:
    ...
    logging.exception("Unable to extract last layer {} to {}, make sure {} is a valid docker image and that the layer path is writable\n".format(
        layer_id, layer_path, tar_path
    ))
    exit(1)
  ...
  try:
    ...
    logging.exception("Unable to write layer Diff ID {} to {}, make sure the path is writeable\n".format(diff_id_digest, diffid_path))
    print()
    exit(1)
